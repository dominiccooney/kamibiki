...
use crate::core::types::{IndexHeader, EMBEDDING_ALIGNMENT, EMBEDDING_BYTES, MAX_HASH_LEN};
use super::{FileEntry};
...
pub fn encode_index(header: &IndexHeader, file_entries: &[FileEntry]) -> Result<Vec<u8>> {
    ...
    let mut buf = Vec::with_capacity(
        HEADER_SIZE
            + file_entries.len() * 6 // rough offset table + chunk counts
            + total_chunks * 2       // lengths
            + EMBEDDING_ALIGNMENT
...
///
/// Positive runs indicate consecutive included git index entries.
/// Negative runs indicate skipped entries. Runs exceeding i16::MAX
/// are split into multiple table entries.
fn encode_offset_table(buf: &mut Vec<u8>, file_entries: &[FileEntry]) {
    ...
    for entry in file_entries {
        ...
        if pos > current_pos {
            // Flush any pending include run, then emit skip.
            if include_run > 0 {
...
/// Write positive (include) run(s), splitting at i16::MAX.
...
/// Write negative (skip) run(s), splitting at i16::MAX.
...
///
/// Returns `(git_index_positions, bytes_consumed)` where
/// `git_index_positions` lists the git index position of each
/// included file, in order.
